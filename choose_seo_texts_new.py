    ...
    for qID in sorted(init_docs.keys(), key=int):
        for dID in init_docs[qID]:
            ...
            for featureName in sorted(feature_id, key=lambda x: feature_id[x]):
                res = 0.0
                if (featureName in normalized_features and qID in normalized_features[featureName] and dID in normalized_features[featureName][qID]):
...
if __name__ == '__main__':
    ...
    for pos in ["2","3","4","5"]:
        ...
        text_df = pd.DataFrame([line.strip().split(None, 2) for line in
                                open(f'./greg_output/saved_result_files/raw_ds_out_{pos}.txt')],
        ...
        df = pd.read_csv(working_set_file_path, delimiter=' ', header=None)
        ...
        df = df.rename(columns={0: 'qid', 2: 'docid'})[["qid", "docid", "score", "rank"]]
