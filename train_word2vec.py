...
FILES_FILE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "W2V/")
RESULTS_FILE_PATH = os.path.join(FILES_FILE_PATH, "w2v_results/")
STOP_WORDS_FILE = os.path.join(FILES_FILE_PATH, "stopwords_list")
UNTRAINED_FILE = os.path.join(FILES_FILE_PATH, "untrained_model.pkl")
...
class Sentences(object):
    ...
    def __iter__(self):
        ...
        for fname in tqdm(os.listdir(os.path.join(FILES_FILE_PATH, self.dirname))):
...
class WordToVec(object):
    ...
    def train_model(self):
        ...
        try:
            ...
            # Check if the untrained_model.pkl file exists

            if not os.path.exists(UNTRAINED_FILE):

                # If it doesn't exist, create the Word2Vec model
                ...
                with open(UNTRAINED_FILE, "wb") as file:
                    pickle.dump(model, file)

            else:
                # If it exists, load the model from the pickle file
                with open(UNTRAINED_FILE, "rb") as file:
...
if __name__ == '__main__':
    ...
    try:
        ...
        LOGS_FILE_PATH = os.path.join(FILES_FILE_PATH, "logs/")
